"""
Parse blastn results into:
  1. blastn_best_hits.tsv          -- one row per (gene, plasmid) pair: the best-
                                scoring HSP, UNFILTERED, for inspecting the
                                identity/coverage distribution before
                                deciding thresholds.
  2. blastn_presence_absence.csv   -- gene x plasmid binary matrix, using the
                                thresholds set below, merged back onto the
                                full original 839-row plasmid table (via
                                accession_mapping.csv) so every original
                                row is represented.
  3. blastn_identity_matrix.csv    -- same shape as (2), but with raw %identity
...
# ----------------------------------------------------------------
BLAST_RESULTS = "output/blastn_results.tsv"
...
best_hits.to_csv("blastn_best_hits.tsv", sep="\t", index=False)
...
final_presence.drop(columns=["_base_acc"]).to_csv("blastn_presence_absence.csv", index=False)
final_identity.drop(columns=["_base_acc"]).to_csv("blastn_identity_matrix.csv", index=False)
